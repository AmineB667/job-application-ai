...
const UI = {
  fr: {
    ...
    breadcrumbRes: "Ressources",
    read: "min de lecture",
    published: "Publié le",
    ctaTitle: "Testez votre CV contre cette offre gratuitement",
    ctaDesc: "Score ATS en 20 secondes, CV refondu + lettre de motivation personnalisée.",
    ctaBtn: "Analyser mon CV →",
    related: "Articles similaires",
    min: "min",
  },
  en: {
    breadcrumbHome: "Home",
    breadcrumbRes: "Resources",
    read: "min read",
    published: "Published on",
    ctaTitle: "Test your CV against this job for free",
    ctaDesc: "ATS score in 20 seconds, rewritten CV + personalized cover letter.",
    ctaBtn: "Analyze my CV →",
      ...
      <div className="container max-w-3xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center gap-4 text-xs text-muted-foreground border-t pt-4">
          <time dateTime={article.date}>
            {t.published} {formatDate(article.date, uiLang)}
          </time>
        </div>
      </div>
      ...
      <main className="container max-w-3xl mx-auto px-6 pb-12">
        ...
        {related.length > 0 && (
          <section className="mt-12 border-t pt-8">
            <h2 className="text-base font-semibold mb-5">{t.related}</h2>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {related.map((r) => {
                const rl = getArticleLocale(r, uiLang);
                return (
                  <Link
                    key={r.slug}
